...
import (
	...
	"github.com/nelobaba/fruitstore_users_api/datasources/mysql/users_db"
	"github.com/nelobaba/fruitstore_users_api/utils/mysql_utils"
	"strings"
)
...
func (user *User) Save() rest_errors.RestErr {
	// used to prepare a query which results a statement that can be used later
	stmt, err := users_db.Client.Prepare(queryInsertUser)
	if err != nil {
		logger.Error("error when trying to prepare save user statement", err)
		return rest_errors.NewInternalServerError("error when trying to save user", errors.New("database error"))
	}
	defer stmt.Close() //defer the close of the statement, to be executed before the function returns
